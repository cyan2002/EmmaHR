...
The following are a list of the best models ranked by AIC
...
Comparing the top 5 models for Gamma vs Gaussian
...
Still top 5 are the same models, but the gaussian and identity link are a better fit. We can try all and look at the diagnostic plots to see if they're good.
...
Mod 7: glm(ABT ~ group + population*Treatment, data = flatlines, family = "gaussian"(link = "identity"))
...
glmer(ABT ~ population*Treatment + (1|day), data = flatlines, family = gaussian, na.action = na.fail)
```{R, echo = FALSE, warning = FALSE}
mod1 <- glmer(ABT ~ population*Treatment + (1|day), data = flatlines, family = gaussian, na.action = na.fail)
summary(mod1)
