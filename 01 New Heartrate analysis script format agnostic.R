...
# Note - only difference between this script and the other script is that it is filename-format agnostic - it will run with filenames that are not formatted the same way as the example data
...
path <- "/Users/chanceyan/Documents/R/EmmaHR/EmmaHR/CardiacF(x)_converted/F2_Trial11_22C_converted"
    ...
    column(12,
           ...
           fluidRow(
             ...
             column(2,
                    ...
                    radioButtons("conf", label = "confidence", choices = list("Good" = "Good","Unsure" = "Unsure", "Discard" = "Discard", "Flatline" = "Flatline"), selected = "Good")),
             column(2,
...
server <- function(input, output) {
  ...
  output$download_hr <- downloadHandler(
    ...
    content = function(file) {
      #hr_data=hr_data %>% separate(file, c("blank","Input","blank2","Trial","Treatment","date","time"), "_")# note: following lines are blanked out to allow the script to be run with any filename format
